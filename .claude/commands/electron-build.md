...
The primary protocol for building or significantly changing an Electron app. Like the orchestrator, this is a **protocol** the main session adopts — the session does the building and dispatches the specialist agents — the framework keeps the hierarchy flat by policy so every dispatch stays auditable.
...
7. **Fix and retest.** If the tester or validator report failures: the main session fixes, then resumes the same tester (it keeps its context and fixtures). Repeat until green or genuinely blocked — never ship on a red run, and never weaken a test to get green.
